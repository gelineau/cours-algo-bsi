...
######################################
# 2.2 Soit une liste de prénoms
# Générer la liste des longueurs des prénoms qui commencent par A (utiliser .startswith)
######################################

first_names = ["Alice", "Bob", "Anna", "Charlie", "Amélie", "David", "Arthur"]


def generate_lengths(names: list[str]) -> list[int]:
    lengths = []
    for name in names:
...
def generate_lengths(names: list[str]) -> list[int]:
    return [len(name) for name in names if name.startswith("A")]


print(generate_lengths(first_names))
...
######################################
# 3. Rechercher à quelle position cette valeur est présente.
# 3.2. Faire aussi une version avec une boucle sans utiliser les fonctions de recherche.
######################################
