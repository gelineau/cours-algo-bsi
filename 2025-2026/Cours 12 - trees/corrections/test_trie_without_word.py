...
from trie_without_word import Trie
...
def test_insert_two_words_with_common_prefix():
    ...
    assert node_a.children["t"].is_end_of_word
    assert node_a.children["n"].is_end_of_word
...
def test_find_words():
    ...
    assert sorted(node.find_words("ap")) == ["ape", "app", "apple"]
